...
public class MessageIntegration extends IntegrationBase {
    ...
    public Mono<MessageDomain> sendTextMessage(Pair<MessageRequest, ChatResponse> pair) {
        ...
        return Mono.just(pair.getFirst())
                ...
                .flatMap(m ->
                        webClient
                                .post()
                                .uri(getUri(pair.getSecond().getBot().getToken(), sendText))
                                .bodyValue(request)
                                .retrieve()
                                .toBodilessEntity()
    ...
    private String getUri(String token, String path) {
        return UriComponentsBuilder.fromUriString(base).path(path).buildAndExpand(token).toString();
    }
}
